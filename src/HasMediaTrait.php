...
trait HasMediaTrait
{
    ...
    public function attachMedia($data)
    {
        if (is_array($data)) {
            foreach ($data as $id) {
                $this->associateItem($id);
            }
        } else {
            $this->associateItem($data);
        }
    }

    public function detachMedia($data)
    {
        if (is_array($data)) {
            foreach ($data as $id) {
                $this->dissociateItem($id);
            }
        } else {
            $this->dissociateItem($data);
        }
    }

    private function associateItem($id)
    {
        $media = app()->make(config('medialibrary.media_model'))->find($id);
        $media->model()->associate($this);
        $media->save();
    }

    private function dissociateItem($id)
    {
        $media = app()->make(config('medialibrary.media_model'))->find($id);
        $media->model()->dissociate();

        $media_item = MediaItem::firstOrCreate(['url' => $media->getFullUrl()]);
        $media->model()->associate($media_item);
